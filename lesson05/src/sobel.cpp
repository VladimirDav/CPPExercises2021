...
cv::Mat sobelDXY(cv::Mat img) {
    ...
    //  исправьте коээфициенты свертки по вертикальной оси y
    int dySobelKoef[3][3] = {
            {-1, -2, -1},
            {0, 0, 0},
            {1, 2, 1},
    ...
    //  доделайте этот код (в т.ч. производную по оси ty), в нем мы пробегаем по всем пикселям (j,i)
    for (int j = 1; j < height-1; ++j) {
        for (int i = 1; i < width-1; ++i) {
            float dxSum = 0.0f; // сюда будем накапливать производную по оси x
            float dySum=0.0f;
...
cv::Mat convertDXYToGradientLength(cv::Mat img) {
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            ...
            float xy = sqrt(dxy[0]*dxy[0]+dxy[1]*dxy[1]); // взяли абсолютное значение производной по оси x

            dxyImg.at<float>(j, i) = xy;
        }
    }
    return dxyImg;
}
